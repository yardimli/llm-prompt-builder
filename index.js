...
// NEW: Saves the current project state (open folders, selected files) to the server.
// This also implicitly sets this project as the "last selected" on the backend.
...
// MODIFIED: Loads a project by fetching its state from the server.
async function loadProject(identifier) {
	...
	try {
		// Fetch the project's saved state (open folders, selected files) from the server.
		const savedState = await $.ajax({
			url: '/',
			method: 'POST',
			data: {
...
// NEW: Restores the UI state (open folders, checked files) from data fetched from the server.
async function restoreState(state) {
	console.log("Restoring state:", state);
	
	// Ensure parent folders of all selected files are marked to be opened.
	...
	// Open folders level by level to build the tree correctly.
	...
	// Check the boxes for the selected files.
...
$(document).ready(function () {
	...
	// NEW: Single initialization function to load all data from the server.
	async function initializeApp() {
		try {
			...
			// 2. Populate Projects Dropdown from server settings
			...
			data.projects.forEach(project => {
				...
				dropdown.append($('<option>', {value: identifier, text: project.path}));
			});
			
			// 3. Load the last project used, or the first in the list.
			if (data.lastSelectedProject && dropdown.find(`option[value="${data.lastSelectedProject}"]`).length > 0) {
				await loadProject(data.lastSelectedProject);
	...
	$('#unselect-all').click(function () {
		$('#file-tree input[type="checkbox"]').prop('checked', false);
		updateSelectedContent();
		saveCurrentProjectState(); // Save the new "all unselected" state
	});
	
	// MODIFIED: Dark mode toggle now saves the setting to the server.
	...
	// MODIFIED: Folder clicks now save the state.
	...
	// MODIFIED: Checkbox changes now save the state.
		...
		try {
			...
			if (response.matchingFiles && response.matchingFiles.length > 0) {
				...
				if (successfulChecks > 0) {
					updateSelectedContent();
					saveCurrentProjectState(); // Save state after search-and-select
